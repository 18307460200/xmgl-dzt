 ...
 * 基础服务类
 */
define("bootstrap/plugins/frame/scripts/service/frameService", function (require, exports, module) {
        ...
        FrameService.prototype.popWindow = function (title, templateNo, param, width, height, isMax, onloadCallBack, ondestroyCallBack, url, allowResize) {
			...
			if($.string.isNotEmpty(width)){
				if(width.indexOf("px") >= 0){
					popWidth = parseInt(width.substr(0,width.length -2)) + 50;
				}else{
					popWidth = parseInt(width) +50 ;
					width = width+"px";
				}
            ...
            window.top.Ewin.dialog({
                ...
                height: height,
				popWidth:popWidth,
				onShown: onloadCallBack,
                onHide: ondestroyCallBack
            }); 
        };
    ...
    FrameService.prototype.queryToken = function (type_token, business_code) {
        ...
        this.invoke(reqParamVo, function (resultVo) {
            if (resultVo.getErrorNo() == 0) {
                ...
                window.top.Ewin.alert(resultVo.getErrorInfo());
            }
        }, false);
        return token;
    };
    ...
    FrameService.prototype.signData = function (param) {
    	...
    	/*var version_id ="";
    	var _version_type="0";
    	var catalog_id ="b2046daa1af248e7bac18be589c60bad";
    	var module_id ="250019d572c9474bbfe206276440ec49";
    	var _app_id="";*/
        if (!param) {
            param = {};
        }
