define("plugins/keyboard/scripts/softkeyboard",function(require, exports, module) {
var $ = jQuery = require('jquery')
require("{platRoot}plugins/keyboard/css/softkey.css");
...
function showkeyboard() {
	...
	softkeyboard.style.left = tleft + "px";
...
function setCalcButtonBg() {
	for ( var i = 0; i < Calc.elements.length; i++) {
		if (Calc.elements[i].type == "button" && Calc.elements[i].bgtype != "1") {
			if (Calc.elements[i].bgtype == "2" || Calc.elements[i].getAttribute("bgtype") == "2") {
			...
			if (thisButtonValue.length == 1) {
				$(Calc.elements[i]).click(function() {
					var thisButtonValue = $(this).val();
					thisButtonValue = thisButtonValue.trim();
					thisButtonValue = jiamiMimaKey(thisButtonValue);
					addValue(thisButtonValue);
				});
				var browser = $.net.getBrowser();
				if(browser.ie){
					$(Calc.elements[i]).dblclick(function() {
...
function capsLockText() {
	if (capsLockFlag) {
		for ( var i = 0; i < Calc.elements.length; i++) {
			...
			var char = char.trim()
			if (Calc.elements[i].type == "button" && char >= "a" && char <= "z"
					&& char.length == 1) {
				Calc.elements[i].value = " "
...
function jiamiMimaKey(newValue) {
	if (typeof (b) == "undefined" || typeof (ifUseYinshe) == "undefined"
			|| ifUseYinshe == 0) {
		return newValue;
	}
	var everyone = '';
...
function showSK(elem)
{
	elem.className='tip_off';password1=elem;showkeyboard();elem.readOnly=1;Calc.password.value=elem.value;
	if(password1.maxLength <= 0){
		password1.maxLength = 1000000000;
	}
}
function resetSK()
{
	password1.readOnly=0;moveEnd(password1);
}
...
window.showSK = showSK;
window.resetSK = resetSK;
window.closekeyboard = closekeyboard;
window.setpassvalue  = setpassvalue ;
window.capsLockText = capsLockText ;
window.OverInput = OverInput ;
window.setCapsLock = setCapsLock ;
var SK = {"showSK":showSK};
module.exports = SK;
});